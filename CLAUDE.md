...
**Status: v11.** This file is read automatically by Claude Code at the start of every session in this repo — keep it at the repo root, and update it whenever a placeholder becomes a locked decision (don't just remember things verbally). **`ChannelSwitcher`'s "Home" is locked as icon-only (see rule 13). Phase 8's three regressions (8.1-fix, 8.3-fix, 8.3-fix-2) are fixed and verified — see plan.md §11. A fourth issue — the navbar rendering fully transparent on product pages, making dark nav text unreadable against the dark navy hero — is queued as the new Prompt 8.7, running next. The former Prompt 8.11 (Vercel deploy for stakeholder review) is renumbered 8.8 and is now the true closing prompt of Phase 8. A new Phase 9 (Stakeholder Comments) follows Phase 8; the former Prompts 8.7–8.10 (performance/SEO through closing QA) are now Phase 10 (10.1–10.4); the former Phase 9 (Payload CMS conversion) is now Phase 11 (11.1–11.4).**
...
18. **Don't start Phase 8 (Final Pre-Deployment Fix Pass) until Phase 7's `docs/remediation-report.md` exists**, and Phase 6's templating-vs-migrate-now decision has been made explicitly with the team. **Don't start Phase 11 (Payload conversion) until Phase 10's `docs/phase10-report.md` exists**, and Phase 9's stakeholder-feedback report confirms the team's review is satisfied.
...
27. **RUM/telemetry (added in Phase 10) must have PII scrubbing configured before it's turned on** — form fields (name/email/phone/business) must never reach a third-party telemetry dashboard via default DOM/session-replay capture. Check this whenever telemetry config changes.
...
33. **A shared chrome element's legibility must never depend on what happens to render behind it.** The navbar going fully transparent on product pages (Phase 8.7) made dark nav text unreadable against a dark hero purely because nothing opaque sat between the two — give shared chrome an explicit, intentional background rather than assuming inherited/transparent styling will look right in every context it's used.
...
- `docs/remediation-report.md` — Phase 7 output; **was never produced** — Phase 7 stopped after item 10.4, item 10.5 never ran. Not retroactively created; folded into Phase 10 §16 (10.3) instead.
- `docs/phase8-report.md` — Phase 8 output (produced by Prompt 8.8); don't start Phase 9 until this exists
- `docs/stakeholder-feedback-report.md` — Phase 9 output; don't start Phase 10 until this exists
- `docs/phase10-report.md` — Phase 10 output; don't start Phase 11 until this exists
- `security-audit-report.md`, `architecture-review-report.md`, `dependency-risk-report.md`, `performance-seo-report.md`, `state-resilience-observability-report.md`, `ui-ux-a11y-report.md` — the six source reports Phase 8/10 remediate (the first three are the *second* version of those reports — re-run against the Phase 7 build, not the originals from Phase 7). Keep all of them for reference; Phase 11's own dependency audit will need re-running against the same categories again.

## Commands

`connect/`: `npm run build:includes` / `npm run build:includes:check` (include-sync, see rule 23). `npm test` / `npm run lint` — **planned since Phase 7, added for real in Phase 10 §16 (10.3)** (check `connect/package.json` directly before assuming either exists). Payload/Next.js commands: fill in once the Phase 11 scaffold exists — do not guess these before the scaffold is created.

## Open items currently in effect (check plan.md §1–2, §5, §9, §10, §11, §12, §13 for full context)
...
- ✅ **All three Phase 8 regressions are fixed** (Prompts 8.1-fix, 8.3-fix, 8.3-fix-2, committed as `8d9df92`): Connect's stale `main.css` cache-busting version was bumped across all 8 pages (a repo-wide sweep found no other stale `?v=` references); the "Funding Solutions" nav trigger now uses explicit literal styles instead of `inherit` and matches its siblings pixel-for-pixel; "Why Choose Us?" now points at `/commercial/about.html` instead of a dead anchor. All verified in-browser.
- 🔴 **A fourth issue found via screenshot review — Prompt 8.7 (Navbar Transparency Fix) — run next:** the navbar renders fully transparent on product pages (confirmed on `commercial/self-employed-home-loan.html`), so dark nav-item text is nearly invisible against the dark navy hero. Fix by giving `.navbar` an explicit, opaque background sitewide (not by changing text color per page), propagate via include-sync, and verify against a fresh screenshot per channel.
- 🔵 **Phase 8 closes with Prompt 8.8** (Vercel deploy for stakeholder review, formerly numbered 8.11) — run immediately after 8.7.
- 🟣 **Phase 9 (Stakeholder Comments, Prompt 9.1) is a placeholder** until feedback comes back from the 8.8 deployment — don't skip it, but don't invent scope for it either.
- 🟢 **Phase 10 (Addressing Reports, formerly Prompts 8.7–8.10, now 10.1–10.4) runs after Phase 9 closes:** 10.1 (image dimensions + WebP + OG/sitemap fixes) → 10.2 (retry/backoff + global error handler + RUM with PII scrubbing) → 10.3 (the never-run Phase 7.13 dependency/test setup + inline-`<style>` consolidation) → 10.4 (final QA + closing report, `docs/phase10-report.md`). Full list in plan.md §13.
- ⚪ **Phase 11 (Payload CMS conversion, formerly Phase 9) waits on Phase 10's closing report** — see plan.md §14 and `prompts.md` Prompts 11.1–11.4.

**Resolved since v10:** the "Why Choose Us" dead-link bug is fixed (8.3-fix-2). A new navbar-transparency bug is scoped and queued as Prompt 8.7. The former Prompt 8.11 (Vercel deploy) is renumbered 8.8 and now genuinely closes Phase 8. A new Phase 9 (Stakeholder Comments) is inserted between Phase 8 and the performance/SEO/resilience/process work, which is renumbered Phase 10 (10.1–10.4). The former Phase 9 (Payload conversion) is renumbered Phase 11 (11.1–11.4) throughout `plan.md`, `prompts.md`, and `buildspec.md`.
