...
**Status: v10 — adds technical detail for two Phase 8 regressions (8.1-fix, 8.3-fix), a third fix (8.3-fix-2, "Why Choose Us" nav link), and resequences 8.11 (Vercel deploy) to run right after 8.6 instead of at the end of Phase 8.** `plan.md` explains *what* and *why*; this explains *how to build it*. `Master Information Architecture & Sitemap.md` is the canonical source of truth for URLs, collection modeling, and routing — this file translates it into concrete build tasks and should never contradict it. If the two ever disagree, the IA doc wins and this file needs updating.
...
**8.11 — Vercel deploy for stakeholder review (resequenced — runs here, right after 8.6, not at the end of Phase 8):** deploy the current static tree (`commercial/`, `connect/`, `personal-and-property/`) to a Vercel project as-is, preserving the existing sub-path structure — this does not require Phase 9's Next.js/Payload scaffold to exist first, since it's deploying the current static HTML/CSS/JS site, not a rebuilt one. Confirm every cache-busting query string is bumped (8.1-fix) before deploying, so the reviewed build reflects every fix through 8.6. Get explicit confirmation before running the deploy, and again before promoting any preview to a production URL. Any final tweaks the team requests here happen before sinking further effort into 8.7's performance/SEO pass and the rest of Phase 8.
