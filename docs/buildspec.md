...
**Status: v11 — renumbering/cleanup pass, matching `plan.md` and `prompts.md`. Phase 8 (§14) now runs 8.1–8.8: the three regression fixes (8.1-fix, 8.3-fix, 8.3-fix-2) are done, a new 8.7 (Navbar Transparency Fix) is added, and the former 8.11 (Vercel deploy) is renumbered 8.8 as the true closing step of Phase 8. A new §15, Phase 9 (Stakeholder Comments), sits between Phase 8 and the old Phase 8.7–8.10 performance/SEO/resilience/QA work, which is now §16, Phase 10 (Addressing Reports), renumbered 10.1–10.4. The old Phase 9 (Payload CMS conversion) — already specified in §1–§9 of this document — is now called Phase 11 throughout; its technical architecture is unchanged.** `plan.md` explains *what* and *why*; this explains *how to build it*. `Master Information Architecture & Sitemap.md` is the canonical source of truth for URLs, collection modeling, and routing — this file translates it into concrete build tasks and should never contradict it. If the two ever disagree, the IA doc wins and this file needs updating.
...
These rules apply specifically to the 60+ static HTML pages produced in Phase 5, before the Payload/Next.js conversion in Phase 11 — they exist because a raw multi-folder HTML build breaks in ways a Next.js app wouldn't.
...
```
No production dependencies added. Re-audit this file (and the new one that appears once Phase 11's Payload/Next.js scaffold exists) with the same five-category dependency analysis before Phase 11's first production deploy. ⚠️ **This §13 block was never actually executed** — see §16 (Phase 10, 10.3) for the follow-through.
...
**8.1-fix — Connect header/hero overlap regression:** the CSS fix landed correctly in `connect/styles/main.css`'s `.hero` rule, but `connect/index.html`'s `<link rel="stylesheet" href="/connect/styles/main.css?v=20260811-1"/>` still carries a stale cache-busting version, predating the fix — any cached copy of that exact URL keeps serving pre-fix CSS. Bump the `?v=` value on every Connect page referencing `main.css` (grep for `main.css?v=` across `connect/*.html`, update every match to the same new value). Also grep the whole repo for any other `?v=YYYYMMDD-N`-style query string and cross-check its date against the file's last real edit (`git log -1 --format=%ad -- <file>`) — bump any that are stale, not just this one. Longer-term: replace this pattern with an auto-bumped hash or a proper `Cache-Control` strategy (already flagged in an earlier architecture pass) rather than continuing to rely on someone remembering to bump a date string by hand.
...
**8.3-fix — "Funding Solutions" trigger visual-parity regression:** `.navbar__link--dropdown-trigger` (`commercial/shared-styles.css`) currently reads `font: inherit; color: inherit;` — theoretically correct, but not visually matching `.navbar__link`'s rendered weight/family in practice. Replace with explicit literal values copied directly from `.navbar__link` in `shared/styles/chrome.css`: `font-family: var(--font-heading); font-weight: 500; font-size: 0.95rem; color: var(--text-primary);` plus the matching `:hover { color: var(--skyblue); }`. Verify against an actual rendered page, not just the stylesheet, since this is exactly the kind of mismatch that reads fine in source but not in the browser. Check any other element touched during 8.3's `<a>`→`<button>` accessibility conversions for the same "relies on `inherit`, doesn't actually match" issue.

**8.3-fix-2 — "Why Choose Us" nav link:** `/commercial/index.html#why` resolves to nothing because `commercial/index.html` has no `id="why"` — the nearest candidate section (`<section class="why-first">`) was never given that id. Rather than adding the missing anchor, point the link at the existing `commercial/about.html` page instead, since it's the fuller "why choose us" content already built and already linked from the footer. Change the `href` once in the canonical `commercial/components/navbar.html`, then re-run `node connect/scripts/build-includes.mjs` / `--check` so it propagates to all 52 affected pages via the same include-sync mechanism used in 8.1 — do not hand-edit individual pages.

**8.4 — CSP/HSTS:** add to `connect/vercel.json`'s existing `headers` array as two more objects in the same `headers` list for the `/(.*)"` source; create equivalent `vercel.json` files at `commercial/` and `personal-and-property/` roots if those are ever deployed as separate projects before Phase 11, otherwise this is moot once Phase 11's single `vercel.json` exists — do the per-channel version now regardless, since Phase 8 is explicitly the last pass before *some* deployment, even if Phase 11 later consolidates it.
...
**8.7 — Navbar Transparency Fix (NEW):** confirmed via direct screenshot of `commercial/self-employed-home-loan.html` and other product pages — `.navbar` renders with no opaque background on these templates, so `.navbar__link`'s dark-navy text (`var(--text-primary)`, set literal in 8.3-fix) sits directly against the page's dark navy hero with nothing behind the nav row to separate the two, making it nearly illegible. Root-cause first: check whether `.navbar`'s background rule is simply absent on product templates, or is being set to `transparent` by an over-broadly-scoped rule intended only for the homepage's "nav floats over the hero image" treatment. Fix by giving `.navbar` an explicit, opaque background sitewide — do not solve this by making nav-item text color conditional on which page/hero is behind it, since that reintroduces exactly the fragility this bug exposed. Propagate via include-sync (`node connect/scripts/build-includes.mjs` / `--check`), since this is shared-chrome CSS, not a per-page override. Verify against a fresh screenshot of at least one product/inner page per channel, not just each channel's homepage (where the bug doesn't reproduce).

**8.8 — Vercel deploy for stakeholder review (formerly 8.11 — now the closing step of Phase 8):** deploy the current static tree (`commercial/`, `connect/`, `personal-and-property/`) to a Vercel project as-is, preserving the existing sub-path structure — this does not require Phase 11's Next.js/Payload scaffold to exist first, since it's deploying the current static HTML/CSS/JS site, not a rebuilt one. Confirm every cache-busting query string is bumped (8.1-fix) before deploying, so the reviewed build reflects every fix through 8.7. Get explicit confirmation before running the deploy, and again before promoting any preview to a production URL. Feedback gathered from this deployment is scoped and addressed in Phase 9 (§15) before Phase 10's performance/SEO/resilience/process work (§16) resumes.

---

## 15. Phase 9 — Stakeholder Comments (NEW, companion to plan.md §12)

No new technical stack or architecture here — this phase is entirely about triaging and implementing feedback gathered from the Phase 8 §14 (8.8) Vercel preview deployment, so its technical shape isn't knowable in advance. When real feedback comes in:

- Treat any fix to something already shipped in Phase 8 the same way as this project has treated every prior regression (8.1-fix, 8.3-fix, 8.3-fix-2) — a targeted, scoped patch to the canonical component, propagated via include-sync, verified in an actual rendered page.
- Treat any genuinely new scope (a new page, a new section, a structural change) as a candidate for explicit deferral to Phase 10 or Phase 11 rather than folding it in here without discussion — this phase exists to catch and log feedback quickly, not to silently become a second full fix pass.
- No `vercel.json`, collection, or component changes are anticipated by default; if the feedback requires them, scope that explicitly against this section before implementing.

---

## 16. Phase 10 — Addressing Reports (formerly Prompts 8.7–8.10, companion to plan.md §13)

Technical detail for each item — full rationale and priority order live in `plan.md` §13. Content and priority are unchanged from the original 8.7–8.10; only the numbering (now 10.1–10.4) and their position relative to Phase 9 changed.

**10.1 — Performance/SEO (formerly 8.7):** image dimension pass is a good candidate for a one-off Node script (`scripts/add-image-dims.mjs`, read each referenced image with a lightweight dimension-reader, inject `width`/`height` into the matching `<img>` tag) rather than 73+ manual edits — consider keeping this script in the repo since new images will keep needing it. WebP conversion likewise scripted (`sharp` or `squoosh-cli` as a one-time devDependency, not a runtime one). OG/sitemap/robots fixes are straightforward per-file additions using Connect's existing files as the template.

**10.2 — Resilience/observability (formerly 8.8):** retry helper as a small `withRetry(fn, {attempts: 3, baseDelayMs: 300})` utility in `connect/scripts/` (or `shared/scripts/`, since P&P/Commercial may eventually need it too), wrapping the two `fetch()` calls in `form.js`. Global error handler and Sentry init both belong in `shared/scripts/`, loaded on every page — Sentry's `beforeSend`/`beforeBreadcrumb` hooks are the mechanism for the PII-scrubbing requirement, not a manual filter written from scratch.

**10.3 — Dependency/process hygiene + CSS consolidation (formerly 8.9):** the exact devDependency block is already specified in §13 above — just actually run it this time, plus `npm install && git add connect/package-lock.json`. For the CSS consolidation, `commercial/index.html`'s 4 `<style>` blocks should merge into 1, with the "LIGHT HERO OVERRIDES (preview variant)" block's rules renamed to a `.hero--light-preview` modifier class and the markup that currently relies on the cascade order updated to include that class explicitly, rather than relying on document-order override.

**10.4 — Closing QA (formerly 8.10):** `docs/phase10-report.md` should reference the six source report filenames directly (`architecture-review-report.md`, `dependency-risk-report.md`, `performance-seo-report.md`, `security-audit-report.md`, `state-resilience-observability-report.md`, `ui-ux-a11y-report.md`) and, for each, list which findings were fixed in Phase 8/10 versus explicitly deferred to Phase 11 with the reason from plan.md §13's final bullet. Also note all four Phase 8 fixes/regressions (8.1-fix, 8.3-fix, 8.3-fix-2, and 8.7's navbar transparency fix) and their resolutions in this report.

---

**Phase 11 — Payload CMS Conversion + Vercel Publish (formerly Phase 9):** fully specified already in §1–§9 above (stack, architecture, collections/globals, URL preservation, per-channel task lists, GoogleReviews, deployment, SEO carry-over) — no separate section needed here, since the technical architecture for this phase hasn't changed, only its number. See `prompts.md` Prompts 11.1–11.4 for the phase-by-phase Claude Code prompts.
