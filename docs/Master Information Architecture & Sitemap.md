...
**Status: v5 — reflects the actual build state after Phase 5 (page production) was run before the header fix landed, widening its blast radius. See the status table below and the matching one in `prompts.md`.**
...
| 5.2 — ChannelSwitcher header fix | 🔴 **Not done — do this next.** Ran Phase 5 before this fix, so the two-tier header bug is now in 62 of 66 HTML files, not just the original set |
| 5.5 — Pre-conversion cleanup | Not done — the spaced-name duplicate folder is already gone, but the retired files (`debtor-finance.html`, `trade-funding-website-application.html`) still need secure deletion |
| 6 — Payload/Vercel conversion | Not started |
